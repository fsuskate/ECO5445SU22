...
d.  Introduce new column to make 

```{r}
applications$'Dept to Income'[which(applications$'Dept to Income' > 100.0)] <- mean(applications$'Dept to Income')
```
...
```{r}
table(applications$Race, applications$'Marital Status')
table(applications$Race, applications$`Years of Education`)
table(applications$Race, applications$`Self Employed`)
table(applications$Race, applications$`Dept to Income`)
table(applications$Race, applications$`Type of Action Taken`)
```
