...
a. Perform a stepwise regression.
...
b. Get ROC curve for the reduced model.


```{r}

prob_reduced <- predict(logit.fit.reduced, applications.validate, type = 'response')

my_roc_reduced <- roc(applications$Approved ~ prob_reduced, plot = TRUE)
...
c. Attempt to use Decision Trees

```{r}
set.seed(1234)

# make the decision tree with the rpart function
dtree <- rpart(applications.train$Approved ~., data = applications.train, method = 'class', parms = list(split = 'information'))

prp(dtree, type = 2, extra = 104,fallen.leaves = T)

```

d. Test the Predictive Accuracy
...
f. Try Using a Conditional Inference Tree

```{r}
fit.ctree <- ctree(Approved ~., data = applications.train)

plot(fit.ctree)
```

g. Prediction and Performance
...
fit.forest <- randomForest(Approved ~ Risk, data=applications.train, na.action=na.roughfix, importance = TRUE)

fit.forest
```

i. Inspect the variable importance.
...
```{r}
forest.pred <- predict(fit.forest, applications.validate)

forest.perf <- table(applications.validate$Approved, forest.pred, dnn = c("Actual","Predicted"))
...
fit.svm <- svm(Approved ~., data=applications.train)

fit.svm
```

l. Prediction and Performance of SVM
...
tuned <- tune.svm(Approved~., data = applications.train,
                  gamma = 10^(-6:1),
                  cost = 10^(-10:10))
tuned
```
...
fit.svm <- svm(Approved~., data=applications.train, gamma = 0.01, cost = 1)

fit.svm
```

o. Prediction and Performance of Tuned Model

```{r}
svm.pred <- predict(fit.svm,na.omit(applications.validate))

svm.perf <- table(na.omit(applications.validate)$class, svm.pred, dnn = c("Actual","Predicted"))

```

p. Select the Best Model
