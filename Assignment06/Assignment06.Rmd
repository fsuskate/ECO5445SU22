...
```{r}
fixed_reg = glm(sale_def ~ bed + bath + area_heated + dist_cbd + dist_lakes + pool + lake_front + downtown + luxury, data=prop_prices)
summary(fixed_reg) 
coef(fixed_reg) 
plot(fixed_reg)
...
```{r}
new_house_df <- data.frame(bed=2,bath=2,area_heated=1223,area=9750,dist_cbd=19368,dist_lakes=490,pool=0)
linear_model <- lm(formula = sale_def ~ bed + bath + area + area_heated + dist_cbd + 
    dist_lakes + pool, data = prop_prices)
predict(linear_model, newdata=new_house_df)
